...
13. **没有日期的笔记只算"写下时成立"** —— 正文没写日期、`valid_at` 也没给（或与写入时间相差不到一天，即
    ...
    日期更早的变更（显式失效、LLM 判为取代、同一单值关系的新值，且新值在笔记写下时仍成立）会让这条笔记
    整条作废（空窗口，`outdatedWhenWritten`），新事实也不再被截在笔记的日期上：先记了一条过时的"住在 X"，
    再补一条"七月从 X 搬到 Y"，现在查到的是 Y。代价是补录一篇比笔记更早、没写结束的旧值文档，LLM 判它取代
    笔记时，笔记同样作废；要避免，在正文写明现状从哪天开始。标记出现之前写入的事实按起点已知处理，
    已经截错的记录用 `reopen_fact`/`invalidate_fact` 手工修。
