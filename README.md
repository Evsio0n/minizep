...
12. **`forget_episode` 撤回不到所有痕迹** —— 被它提前的事实起点不改回，实体标签不改；它最后写的实体摘要
    ...
    不自动重开，摘要不改回。
13. **没有日期的笔记只算"写下时成立"** —— 正文没写日期、`valid_at` 也没给（或与写入时间相差不到一天，即
    agent 填的"现在"）时，事实的起点只是笔记写下的时刻，记为 `attributes.startFromEpisode`。之后录入的
    文本自己写明这条关系在笔记之前哪天结束（带日期、指明关系的失效）时，笔记写下时就已过时：整条作废
    （空窗口，`outdatedWhenWritten`），新事实也不再被截在笔记的日期上。先记了一条过时的"住在 X"，再补一条
    "七月从 X 搬到 Y"，现在查到的是 Y。只是与笔记冲突的更早的值不会让它作废——LLM 判为取代、同一单值
    关系的新值、没写日期的失效、不指明关系的"分开了"、同一文本里记着已经结束的那段经历（"2015 到 2017 年
    在 X"）：晚学到的更正和晚补录的旧文档在这里分不出来，这个值照旧在笔记开始处结束，笔记仍是现在的值。
    剩下的代价：补录一篇只写"2017 年离开了 X"的旧文档，一条没写日期、其实说的是后来重新入职的"在 X 工作"
    同样作废；要避免，在正文写明现状从哪天开始。`forget_episode` 忘掉更正时笔记重新成立，但笔记自己的
    episode 先被忘掉的话它保持撤回。标记出现之前写入的事实按起点已知处理，已经截错的记录用
    `reopen_fact`/`invalidate_fact` 手工修。
